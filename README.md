...
```
maps $x_1 \equiv x(t=1) \sim p(x_1)$ to $x_0 \equiv x(t=0) \sim p(x_0)$ *deterministically*, where $p(x_1)$ is a diagonal $d$-dimensional standard normal density and $p(x_0)$ is a target $d$-dimensional density for which we have a point cloud, $\\{ x_0^{(j)}\\}_{j=1}^n$, which, in a typical application, is generated using a simulator. The probability density $p(x)$ smoothly interpolates between $p(x_1)$ and $p(x_0)$ and $S(t, x)$ is the associated **score function**.  Using the definitions of the functions $f(t)$ and $g(t)$ in Ref.[1], and after some manipulation, these equations can be written as
```math
\begin{aligned}
    \frac{d x}{dt} 
     & = \lambda(t) \, x + \mu(t)
